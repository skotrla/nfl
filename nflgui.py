...
if db[0] == '':
        ...
        if len(mydate[0]) > 0:
            if len(mydate[0]) >= 10:
                nfl = pd.read_sql(f'SELECT g1.* FROM games g1 INNER JOIN (SELECT Week, Year, RTeamN, MAX(Date) as Date FROM games WHERE Date <= "{mydate[0]}" GROUP BY Week, Year, RTeamN) g2 ON g1.Week=g2.Week AND g1.Year=g2.Year AND g1.RTeamN=g2.RTeamN AND g1.Date=g2.Date',connection).drop(columns=['index'])
                nflb = pd.read_sql(f'SELECT g1.* FROM games g1 INNER JOIN (SELECT Week, Year, RTeamN, MAX(Date) as Date FROM games WHERE Date <= "{mydate[0]}" GROUP BY Week, Year, RTeamN) g2 ON g1.Week=g2.Week AND g1.Year=g2.Year AND g1.RTeamN=g2.RTeamN AND g1.Date=g2.Date',connection2).drop(columns=['index'])
            else:
                nfl = pd.read_sql(f'SELECT g1.* FROM games g1 INNER JOIN (SELECT Week, Year, RTeamN, MAX(Date) as Date FROM games WHERE DATETIME(Date,"+{mydate[0]} hours") <= DATETIME(GameDate,"-5.5 hours") GROUP BY Week, Year, RTeamN) g2 ON g1.Week=g2.Week AND g1.Year=g2.Year AND g1.RTeamN=g2.RTeamN AND g1.Date=g2.Date',connection).drop(columns=['index'])
                nflb = pd.read_sql(f'SELECT g1.* FROM games g1 INNER JOIN (SELECT Week, Year, RTeamN, MAX(Date) as Date FROM games WHERE DATETIME(Date,"+{mydate[0]} hours") <= DATETIME(GameDate,"-5.5 hours") GROUP BY Week, Year, RTeamN) g2 ON g1.Week=g2.Week AND g1.Year=g2.Year AND g1.RTeamN=g2.RTeamN AND g1.Date=g2.Date',connection2).drop(columns=['index'])                
        else:
            nfl = pd.read_sql(f'SELECT g1.* FROM games g1 INNER JOIN (SELECT Week, Year, RTeamN, MAX(Date) as Date FROM games GROUP BY Week, Year, RTeamN) g2 ON g1.Week=g2.Week AND g1.Year=g2.Year AND g1.RTeamN=g2.RTeamN AND g1.Date=g2.Date',connection).drop(columns=['index'])
        ...
        nfl['GameDate']=np.where(nfl['GameDate'].dt.month == 9, nfl['GameDate'] + td(hours=1),nfl['GameDate'])
        nfl['GameDate']=np.where(nfl['GameDate'].dt.month == 10, nfl['GameDate'] + td(hours=1),nfl['GameDate'])
        ...
        nfl['OWin'] = 0
        nfl['OWin'] = np.where(nfl['Total'] >= nfl['BTotal'], nfl['TActual'] - nfl['BTotal'],nfl['OWin'])
        nfl['OWin'] = np.where(nfl['Total'] < nfl['BTotal'], nfl['BTotal'] - nfl['TActual'],nfl['OWin'])
        nfl['OWin'] = np.where(nfl['OWin'] > 0, 1, nfl['OWin'])
...
#DBFIX
        ...
        bga2 = pd.read_sql(f'SELECT "table",Date,player as name, Map FROM arknova', connection)
        bga2b = pd.read_sql(f'SELECT "table",Date,player as name, Map FROM arknova', connection2)
...
#        bga['Date'] = pd.to_datetime(bga['Date']).dt.strftime('%Y-%m-%d')
        ...
        bga['Date'] = np.where(bga['Date'].isnull(), pd.to_datetime((bga['Date'].min() + pd.TimedeltaIndex(pd.to_timedelta(bga['date_delta'], unit='D'))).astype('str').str[:10]),bga['Date'])
...
#        lastdate = dates['Date'].max()        
        ...
        if c[0] == 'map':
            ...
            st.bar_chart(data=fdf.groupby(['name','map']).sum(numeric_only=True).reset_index(), x='map', y='elo change', color='name', stacked=False, width="stretch", height="content")
        else:
